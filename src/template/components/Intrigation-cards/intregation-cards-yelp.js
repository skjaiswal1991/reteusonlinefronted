...
class IntregationCardsYelp extends Component {
  constructor(props) {
    super(props);
    this.state = {
      show: false,
  ...
  subscribehendler = () =>{
    DataLayer.createAgreegation({businessId:this.state.businessId,yelp:this.state.bussinessReviews})
      .then((res)=>{
  ...
  handleShowOther = (data) => {
    ...
    dataLayer.getyelpData(this.state.bussiness)
    .then((review)=>{
        console.log(review);
        ...
        if(review.data.status == 'success'){
          ...
          this.setState({
            bussinessReviews:{
              ...
              address:review.data.result.location.address1,
              user_rating: review.data.result.rating,
              user_review: review.data.result.review_count,
              photos_url: review.data.result.image_url,
...
export default connect(mapStateToProps, mapDispatchToProps)(IntregationCardsYelp);
