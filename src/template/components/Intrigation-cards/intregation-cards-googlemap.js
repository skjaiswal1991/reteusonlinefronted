...
class IntregationCardsGooglemap extends Component {
  constructor(props) {
    super(props);
    this.state = {
      show: false,
  ...
  subscribehendler = () =>{
    DataLayer.createAgreegation({businessId:this.state.businessId,googlemap:this.state.bussinessReviews})
      .then((res)=>{
  ...
  handleShowOther = (data) => {
    ...
    dataLayer.getgoogleMapData(this.state.bussiness)
    .then((review)=>{
        console.log(review);
        ...
        if(review.data.status == 'success'){
          ...
          this.setState({
            bussinessReviews:{
              ...
              user_review: review.data.result.review,              
...
export default connect(mapStateToProps, mapDispatchToProps)(IntregationCardsGooglemap);
