...
class IntregationCardsAmazon extends Component {
  constructor(props) {
    super(props);
    this.state = {
      show: false,
  ...
  subscribehendler = () =>{
    DataLayer.createAgreegation({businessId:this.state.businessId,amazon:this.state.bussinessReviews})
      .then((res)=>{
  ...
  handleShowOther = (data) => {
    ...
    dataLayer.getAmazonMapData(this.state.bussiness)
    .then((review)=>{
        console.log(review);
        ...
        if(review.data.status == 'success'){
          ...
          this.setState({
            bussinessReviews:{
              name: review.data.result.title,
              url: review.data.result.url,
              photos_url: review.data.result.img,
...
export default connect(mapStateToProps, mapDispatchToProps)(IntregationCardsAmazon);
